...
- Organization:
- Name: This field is autocompleted according to the options selected in the Table and Mapping Type fields.
- Table:
- Mapping Type: two options: Etendo to external system and External system to Etendo.
- Is Rest Endpoint: Checkbox, the endpoint is generated
- External_Name: Name to enter the API. It must be unique, it can't be repeated.
- Active: A check to indicate whether this record is available or disabled.

#### Create Projection Fields

With this button, a pop up window is shown where you can find a list of all the fields to project. You can select the fields that are necessary to project.

!!!info
  The mandatory fields are already selected by default.
...
- Field Mapping: 
  ...
  - Constant Mapping: If you choose this option, the same defined constant value is always shown

  - Create related (Only for WRITE)


Extra field shown when Entity Mapping option above is selected: RELATED PROJECTION ENTITY
- Jsonpath: language to extract properties from Json objects. + Link a Jsonpath info
- Constant Value: Only available when Constant Mapping is selected as the Mapping type
- Related Projection Identifiers: information field to revise the identifier of the mapped table.
- Active: A check to indicate whether this record is available or disabled.

MAP FIELDS TO CHILD (From WRITE)

- It provides information from the parent fields to the child records.
