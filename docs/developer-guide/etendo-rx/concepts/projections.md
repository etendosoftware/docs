...
When Etendo retrieves data from the database, it often returns more fields than a given integration needs. Projections let you define exactly which fields are included, reducing the amount of data transferred and improving performance. (Technical detail: this is implemented using Spring Data JPA projections on the persistence layer.)
...
Projections and mappings are used to control which Etendo data fields are shared with, or received from, an external system — such as an e-commerce platform or a marketplace. The connection between Etendo and that external system is made through an API (a standard communication channel between software applications). Two common scenarios are:

- **Exposing Etendo data to an external system**: an e-commerce platform reads product or order data from Etendo through a read-only API connection generated by the DAS service.
- **Importing data from an external system into Etendo**: customer or order data from a marketplace is sent into Etendo Classic through a write API connection.

Use projections to control exactly which fields are exposed or consumed, keeping the integration surface minimal and explicit.
...
!!!info
    **This window is the central place to create data reading and writing endpoints in Etendo.**

This window uses the [DAS service](../getting-started.md) (Data Access Service — an Etendo RX background service that reads the configuration you define here and automatically creates the API connection points used to exchange data with external systems). The DAS service reads the configuration defined here and generates the corresponding endpoints when it is launched.
...
!!!info
    Whether you need one or both projection types depends on your integration direction: create a **Read** type to expose Etendo data to an external system, a **Write** type to import external data into Etendo, or both if you need bidirectional integration.
...
This window also allows the creation of search endpoints, using the [DAS service](../getting-started.md) (Data Access Service — an Etendo RX background service that reads the configuration you define here and automatically creates the API connection points used to exchange data with external systems). This service, based on this window configurations, dynamically generates the endpoints when launched.

### Repository Tab

The Repository tab is used to register a table so the DAS service can generate filtered search connections for it — for example, retrieving only orders belonging to a specific business partner. Registering a repository here enables the [DAS service](../getting-started.md) (Data Access Service — an Etendo RX background service that reads the configuration you define here and automatically creates the API connection points used to exchange data with external systems) to generate custom filtered search connections — beyond the standard create, read, update, and delete operations — for the selected table. Use the Repository tab when you need filtered queries (for example, "get all orders for a specific business partner") that are not covered by the default connections.

To register a repository, go to Tables and Columns, select a table and, in the *Repository* tab, create a new record defining the module in development where the configurations are exported.
...
In this window, it is possible to define constant values to be used for [entity fields](#entity-field-subtab) in which the information must not be modified. This configuration is done in the [Projections and Mappings](#projections-and-mappings-window) window.
