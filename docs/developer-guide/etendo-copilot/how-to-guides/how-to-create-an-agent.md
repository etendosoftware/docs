...
!!! tip
    - **Remember the Synchronization**: After adding/modifying/deleting a knowledge base file from an Agent, it's necessary to synchronize the agent to apply the changes. This not only regenerates/reloads the Knowledge Base File but also updates the Agent with the latest changes.
    - **Splitting**: When the indexation in the knowledge base file is done, the content is split into chunks depending on the type of the file. For example, if the file is a markdown file, the content is split into chunks by the headers. If the files are not large, it's possible to mark as `Skip Splitting` in the knowledge base file configuration. This will avoid the splitting of the content into chunks. This causes the content of the documents to be retrieved as a single chunk, which can be useful in some cases.
