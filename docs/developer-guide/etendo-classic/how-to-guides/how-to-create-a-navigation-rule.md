...
These rules are defined in the **Navigation Rules** tab of the **Windows, Tabs and Fields** window, below the **Field** tab. System administrator can add rules to a field in order to make it navigate to a custom tab.
...
The rules at table level for the **Extended Navigation Model** are defined in a new tab of the **Tables and Columns** window called **Navigation Rules**. System administrator can add new rules to a table in order to make the links which have that table as a reference navigate to a custom tab.
