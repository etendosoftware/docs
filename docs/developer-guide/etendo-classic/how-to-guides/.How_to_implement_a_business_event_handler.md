...
Some notes on business entity events:
  ...
  * you can make use of the full [data access layer](concepts/Data_Access_Layer) functionality in your event handling code, you can query, create new objects, persist etc. 
...
Business events make use of the event framework provided by the [Weld](concepts/Etendo_Architecture#Introducing_Weld:_dependency_injection_and_more)
framework. To register an event handler, annotations are used. Weld
automatically analyzes the classpath and automatically detects classes/methods
with annotations.

!!!note
    In order to maximize performance, certain part of the classpath are excluded, check out [this section](concepts/Etendo_Architecture#Analyzing_the_classpath) if your event handlers are not found.

In this section, we will implement an event handler on the Greeting entity.
Whenever a title is saved, a Spanish translation will be added. In addition, we
...
This section is supported by an example module which shows an example of the code
shown and discussed here.

The code of the example module can be downloaded from [this]() repository. (LINK THE REPOSITORY WHERE THE EXAMPLE IS CREATED AND THEN DELETE THIS COMMENT)

The complete source code of the event handler is available [here]() (LINK THE SOURCE CODE OF THE EVENT HANDLER HERE AND THEN DELETE THIS COMMENT).
...
When you add the above class to your module, restart the system, then go
...
!!!note
    ...
    * Within the event handler methods, you can use the api on the event object to detect which is the entity event and to get access to the current and previous state of the entity. See [here](concepts/Etendo_Architecture#Event_Classes_and_API) for more information. 
...
The `getObservedEntities` method is called when you call the `isValidEvent` method, `getObservedEntities` returns the entities you want to handle in this event handler. To make use of it, add this code to each event handling method in the beginning:
...
The changed entity instance does not need to be saved explicitly, this is done by the [data access layer](concepts/Data_Access_Layer) and hibernate automatically.

Then test the changes, go to the window (http://localhost:8080/openbravo/?tabId=282 ), enter a new greeting without a dot in the title. When saving, you will see a dot getting added. Try updating the record, you will have the same behavior.
...
properties are set. Note that as the object is not part of the event, you can call
its setters directly. The language is arbitrarily chosen. See the DAL
document for information on the APIs you can use to retrieve objects from the
database.
...
Sometimes you need to interrupt the save action because the user is doing
...
This work is a derivative of [How to Implement a Business Event Handler](http://wiki.openbravo.com/wiki/How_to_implement_a_business_event_handler){target="\_blank"} by [Openbravo Wiki](http://wiki.openbravo.com/wiki/Welcome_to_Openbravo){target="\_blank"}, used under [CC BY-SA 2.5 ES](https://creativecommons.org/licenses/by-sa/2.5/es/){target="\_blank"}. This work is licensed under [CC BY-SA 2.5](https://creativecommons.org/licenses/by-sa/2.5/){target="\_blank"} by [Etendo](https://etendo.software){target="\_blank"}.
