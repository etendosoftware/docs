---
title: How To Log Using Log4j1.x
tags: 
    - log
    - log4j1.x
    - howto
...
Etendo includes [log4j](https://logging.apache.org/log4j/1.x/){target="\_blank"} as logging framework, so all logs should be written making use of it.
...
Alternatively, it is possible to use [slf4j](https://www.slf4j.org/){target="\_blank"} API which finally will also use log4j. It provides, for example, param substitution. The following code is equivalent to previous:
...
Be careful when logging context not to create new exceptions. For example, the following log line could throw a ` NullPointerException ` in case ` invoice ` variable is ` null ` when it is executed: ` log.debug("Processing invoice",
**invoice** .getId()); `
...
This work is a derivative of [How To Log Using Log4j1.x](http://wiki.openbravo.com/wiki/How_To_Log_Using_Log4j1.x){target="\_blank"} by [Openbravo Wiki](http://wiki.openbravo.com/wiki/Welcome_to_Openbravo){target="\_blank"}, used under [CC BY-SA 2.5 ES](https://creativecommons.org/licenses/by-sa/2.5/es/){target="\_blank"}. This work is licensed under [CC BY-SA 2.5](https://creativecommons.org/licenses/by-sa/2.5/){target="\_blank"} by [Etendo](https://etendo.software){target="\_blank"}.