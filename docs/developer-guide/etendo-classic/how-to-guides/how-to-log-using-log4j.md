...
!!!warning
    ...
    They write log in a different file `catalina.out`, which has several inconveniences:

    - Having a single source of logs makes much easier to look for problems. 
    - ` catalina.out ` is not automatically rotated, so it is size might increase without limit. 
...
`OBException` is an unchecked exception. 
