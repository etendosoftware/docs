...
!!!info
    It is possible to specify an external connection provider that Etendo will use to obtain the JDBC connections. For that, a module containing a subclass of `ExternalConnectionPool` needs to be installed, and the `db.externalPoolClassName` property has to be set in `Openbravo.properties`.

##  Example: Using the Apache JDBC Connection Pool

The Apache JDBC Connection Pool Etendo modules provides an implementation of the Apache JDBC Connection Pool.
...
This module contains a configuration file template: `modules/org.openbravo.apachejdbcconnectionpool/config/connectionPool.properties.template`. In order to customize the JDBC connection pool properties this file has to be copied to `modules/org.openbravo.apachejdbcconnectionpool/config/connectionPool`.properties. The user can then configure the pool properties according to his needs. Hints about how to configure this properties can be found [here](https://tomcat.apache.org/){target="\_blank"}.
