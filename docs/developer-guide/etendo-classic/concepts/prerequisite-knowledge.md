...
* [Gradle](https://gradle.org/){target="\_blank"}: The tool used to execute the build tasks related to Etendo like compiling, deployment to an application server, exporting and importing the database schema, etc. 
* [IntelliJ IDEA](https://www.jetbrains.com/idea/){target="\_blank"}: An integrated development environment that helps in all the development tasks related to Etendo like editing source files, debugging, executing build tasks, etc. 
