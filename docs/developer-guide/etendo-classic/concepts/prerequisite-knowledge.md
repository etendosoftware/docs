...
* Java  : The programing language used in all the server side components of Etendo. It is used in all levels from the core components to ERP business logic. 
...
* Apache Ant  : The tool used to execute the build tasks related to Etendo like compiling, deployment to an application server, exporting and importing the database schema, etc. 
* Eclipse IDE  : An integrated development environment that helps in all the development tasks related to Etendo like editing source files, debugging, executing build tasks, etc. 
