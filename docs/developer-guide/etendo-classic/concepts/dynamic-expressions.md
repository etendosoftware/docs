...
##  Session Values (Session Variables)
:material-menu:  `General Setup` > `Application` > `Session Variables`

Depending on how they are defined, there are different kinds of session values. It is possible to check (as **System Administrator**) all current values loaded in session in `General Setup` > `Application` > `Session Variables` window.
...
For a list of current system preferences, visit [Preference](../../../user-guide/etendo-classic/basic-features/general-setup/application/preference.md) page.
...
:material-menu: `Application Dictionary` > `Setup` > `Auxiliary Input`
...
It is used to obtain a boolean value. It does not have any prefix. Its syntax is `Javascript` but the comparators are:
