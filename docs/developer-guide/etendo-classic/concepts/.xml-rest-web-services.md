...
Etendo REST consists of a framework offering security and exception services and a data access REST web service implementation. The data access (DAL) REST web services provide a CRUD-like web service so that external applications can retrieve, update, create and delete business objects through
standard HTTP requests.

## Glossary

This document uses the term business object to denote an entity and its dependent information. A business object can be a simple entity such as a currency which just has basic primitive fields. On the other hand it can also be a structure of entities, for example an order header with its order line. A
business object structure always has one business object which is the owner of all the business objects in that structure, for example for a sales order the sales order header business object is the owner of the complete structure. Another way of describing this is that the order lines depend on the order
header, i.e. an order line can not exist without its order header and when an order header is removed also the order line should be removed.
...
- allows linking to specific business objects or to sets of business objects. This is a very powerful feature of a REST approach and it allows for easy navigation between business objects.
...
The DAL REST web service is an implementation of a REST web service in Etendo. It provides a CRUD-like web service so that external applications can retrieve, update, create and delete business objects through standard HTTP requests.
...
- retrieve a list of business objects using just the type, for example [https://demo.etendo.cloud/etendo/ws/dal/Country](https://demo.etendo.cloud/etendo/ws/dal/Country) (User Name: admin / Password: admin)
...
[https://demo.etendo.cloud/etendo/ws/com.smf.securewebservices.obrest/Country/106](https://demo.etendo.cloud/etendo/ws/com.smf.securewebservices.obrest/Country/106) (User Name: admin / Password: admin)
...
To query on date fields for a specific date it is easiest to use a larger/smaller than, this because date fields are stored in the database with a time value (which can include milliseconds):
...
!!!note
    ...
    [https://demo.etendo.cloud/etendo/ws/dal/Country?where=namelike%20'%25Fr%25'](https://demo.etendo.cloud/etendo/ws/dal/Country?where=namelike%20'%25Fr%25) (User Name: admin / Password: admin)
...
!!!note
    ...
    is also possible to query the server using the currency identifier returned. Nevertheless, for this case we must use the original name of the identifier, in the currency table it is ISO_CODE so the query would change to:
...
!!!note
    ...
    Notice that the where clause can also be used with more than one condition using the AND clause, e.g.
...
Using the parameter `_selectedProperties` it is possible to specify the list of properties that the webservice should return. This parameter can be used both in single and query requests.
...
The use of the `_selectedProperties` parameter has a benefitial effect on the performance: the fewer child entities are specified, the better the performance of the webservice call will be.
...
[https://demo.etendo.cloud/etendo/ws/dal](https://demo.etendo.cloud/etendo/ws/dal) (User Name: admin / Password: admin)    
...
Also, the default role for the user who is doing the log in should be enabled to do calls to web services. Search for "Is Web Service Enabled" inside the [Role](../../../user-guide/etendo-classic/basic-features/general-setup/security/role.md#role) document for more information.

After a successfull login the REST web service action is run within the context of the user. The REST webservice uses the [DAL security control](../concepts/data-access-layer.md#security-and-validation) mechanism to control read and write access.

## Stateless Webservice Requests - HTTP Session

!!!note
    This is important for high frequency webservice calls.  
  
By default OB webservice requests are statefull. Meaning that each webservice call will create and use a http session in the OB server. Depending on how you setup the client to call the webservice this can even mean that each webservice request creates a new http session. This is not advisable for high-
...
- passing the parameter stateless=true in the request url. For example: [https://demo.etendo.cloud/etendo//ws/org.openbravo.howtos.doIt?product=1000001&stateless=true](https://demo.etendo.cloud/etendo//ws/org.openbravo.howtos.doIt?product=1000001&stateless=true)
...
A REST web service request is basically the same as a normal HTTP request. This means that transaction handling is the same as for standard HTTP requests. If no exception occured then the transaction is committed at the end of the web service request (on the Etendo server).
...
The DAL webservice generates a XSD schema on the basis of the runtime model. This XSD schema defines the xml which is generated by the REST web service. This definition is also used for the XML used to update Etendo through REST web services.

For an Etendo instance running on `https://demo.etendo.cloud/etendo`, the XSD can be retrieved through this request: [https://demo.etendo.cloud/etendo/ws/dal/schema](https://demo.etendo.cloud/etendo/ws/dal/schema) (User Name: admin / Password: admin)
    
## Error Handling

When a request can not be processed and an error occurs then an error XML document and the relevant HTTP response code is returned. The error XML document contains the exception message. The stacktrace and other more detailed information is not sent out on purpose (because of security reasons).

## Simple XSLT templates

Etendo has some simple XSLT templates to show how the XML can be processed and demonstrate that REST is a good approach for allowing navigation through data sets. The following URL shows the start URL for a DAL web service call using the XSLT templates:

- [https://demo.etendo.cloud/etendo/ws/dal/?template=types.xslt](https://demo.etendo.cloud/etendo/ws/dal/?template=types.xslt) (User Name: admin / Password: admin)
- [https://demo.etendo.cloud/etendo/ws/dal/Country/106?template=bo.xslt](https://demo.etendo.cloud/etendo/ws/dal/Country/106?template=bo.xslt) (User Name: admin / Password: admin)
- [https://demo.etendo.cloud/etendo/ws/dal/Currency/100?template=bo.xslt](https://demo.etendo.cloud/etendo/ws/dal/Currency/100?template=bo.xslt) (User Name: admin / Password: admin)
    
The XSLT templates can be found in the `org.openbravo.service.rest` package.

## Test

The REST Webservice functionality is tested using junit testcases. REST test cases can be found in the Etendo development project in the src-test folder and then in the `org.openbravo.test.webservice` package. The REST test cases give a first impression on how to do REST requests directly from Java includede basic authentication.

## Tips & tricks and Troubleshooting

For troubleshooting and tips and tricks, please see this link [here](../concepts/common-issues-tips-and-tricks.md#rest-web-services). The tips and tricks section for example discuss a firefox plugin to make it easy to test REST webservices.
