...
[JSON_REST_Web_Services](./json-rest-web-services.md) functionality.
...
[JSON_REST_Web_Services](./json-rest-web-services.md). So the datasource at runtime operates very much like the JSON webservice. There are, however, some minor differences:

  * When only a single object is [requested](./json-rest-web-services.md#get), then JSON REST will return only a JSON string with this object. The datasource will wrap this single JSON string and add metadata required for the client-side datasource. 
...
When developing your own datasource, it makes sense to study and use the [core classes](./json-rest-web-services.md#json-core-classes) of the JSON REST functionality.
