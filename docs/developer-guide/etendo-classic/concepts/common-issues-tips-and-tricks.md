...
This check can be done either for core or for any module by running the following script: `modules/org.openbravo.client.kernel/jslint`
...
It can help to have the Smartclient source code and reference material in your IntelliJ workspace, it is very easy to search source code then. The latest Smartclient source code is available in this repository: [https://bitbucket.org/koodu_software/org.openbravo.userinterface.smartclient.dev](https://bitbucket.org/koodu_software/org.openbravo.userinterface.smartclient.dev)
...
3. Go to Manage(Setting icon) in the bottom-left corner and select `Settings`.
...
Yes, you can see [Etendo Architecture](../concepts/etendo-architecture.md#implement-application-initialization-logic) for more information.

##  Data Access Layer

###  SQL Functions in HQL: no data type for node error

Visit [Data Access Layer](../concepts/data-access-layer.md#sql-functions-in-hql) for information on how to use SQL functions in HQL.
...
Etendo contains a  module  which offers a query tool which allows you to try a **HQL Query** directly in the Etendo interface. The [HQL Query Tool](https://github.com/etendosoftware/org.openbravo.utility.hqlquerytool) is also available through the Etendo repository. 

###  How can I see what SQL is executed?

To see the SQL queries fired by Hibernate, the following log4j property has to be set set in `WEB-INF/log4j2-web.xml` or in `config/log4j2-web.xml`:
...
If it does not work for you, it is possible that the log4j.properties file is read from another location than you expect. To see where log4j reads its properties from set this jvm parameter: `-Dlog4j2.debug=true`
...
This query seems pretty simple. However, in fact the image is a separately persisted object and a any-to-one reference from the product to the image table. This query therefore results in an inner-join to the image table. In this case that table was empty and nothing was returned.
...
If your REST webservice only returns this: `<ob:Openbravo/>`
