...
Cuando necesitas utilizar un agente de IA para realizar tareas con un alto volumen de iteraciones, estará limitado en la cantidad que puede manejar y en su velocidad. Entonces nace el concepto de tareas masivas, que consiste en almacenar solicitudes en una ventana del módulo `Tasks`.

!!!warning
    Es necesario tener en cuenta que estas tareas deben estar *aisladas* entre sí. Es decir, no necesitan información unas de otras. Por ejemplo, al cargar datos desde un archivo de Excel, cada fila se ejecutará por separado.

Estas solicitudes pueden ejecutarse manualmente o procesarse mediante un proceso en segundo plano ya incluido en el módulo Copilot.
...
- **Question/Request**: La descripción o solicitud que se usará como base de la tarea. En el caso del agente `Bulk Task Creator`, este se encargará de reformular la tarea para convertirla en tareas singulares. Pero para las otras opciones, es necesario proporcionar la base de la tarea en forma singular.
- **Execution group**: Grupo opcional. Si no se establece, utiliza el ID de la conversación. Se usa para identificar las tareas que pertenecen al mismo grupo.
- **Task Type**: ID de tipo de tarea opcional. Si no se establece, crea automáticamente uno llamado "Copilot". Se usa para identificar el tipo de tarea.
- **Estado**: ID de estado opcional. Por defecto es "Pendiente". Se usa para identificar el estado de la tarea. Después de que la tarea se procese, el estado se actualizará a `Completed`.
...
2. Haz clic en el botón `Add Copilot Task`. Esto abrirá una ventana para establecer los parámetros de las tareas masivas.
    ...
    1. Establece el parámetro **Question/Request** con la descripción de la tarea, en forma singular, que se usará como base de la tarea. Por ejemplo, `Create product with this data:`.
    ...
    5. Establece un **Elemento separador**. Este es el carácter que se usará para separar los datos del archivo CSV. Por ejemplo, `,` o `;`.
...
4. Envía una solicitud como:
    ...
    !!! info
        Este uso de la `Task Creator Tool` es un ejemplo; el agente puede tener supuestos o simplificaciones que pueden cambiar la forma de usarla. Esto puede hacerse personalizando el prompt del agente.
...
- **Los registros relacionados deben procesarse juntos**: Por ejemplo, todas las líneas de un pedido de venta deberían procesarse como una única tarea de creación del pedido.
...
- **Reducir la sobrecarga de tareas**: En lugar de crear cientos de tareas individuales, puedes crear menos tareas con datos agrupados.
...
Cada tarea ahora contiene un array JSON con todas las líneas de ese pedido específico, permitiendo al agente procesar el pedido completo en una sola ejecución.

#### Modo Preview

Antes de crear un gran número de tareas, puedes usar el parámetro `preview` para validar la estructura de la tarea:
