---
title: Etendo - Imagen ISO
tags:
    - Notas de la versión
    - Etendo
    - Versiones
    - ISO
---

# Etendo  - Imagen ISO

:material-tools: Guía de instalación: [Guía de instalación de ISO](../../../getting-started/installation.md#steps-to-install-the-iso-with-internet-connection)

| Versión | Fecha de publicación | Imagen ISO |
| --- | --- | --- |
| [26.1.1](#2611) | 09/04/2026 | [26Q1.1.iso](https://etendo-appliances.s3.eu-west-1.amazonaws.com/etendo/iso/etendo-26Q1.1.iso) |
| [25.4.3](#2543) | 22/01/2026 | [25Q4.3.iso](https://etendo-appliances.s3.eu-west-1.amazonaws.com/etendo/iso/etendo-25Q4.3.iso) |
| [25.4.0](#2540) | 29/12/2025 | [25Q4.0.iso](https://etendo-appliances.s3.eu-west-1.amazonaws.com/etendo/iso/etendo-25Q4.0.iso) |
| [25.3.0](#2530) | 30/09/2025 | [25Q3.0.iso](https://etendo-appliances.s3.eu-west-1.amazonaws.com/etendo/iso/etendo-25Q3.0.iso) |
| [25.2.0](#2520) | 02/07/2025 | [25Q2.0.iso](https://etendo-appliances.s3.eu-west-1.amazonaws.com/etendo/iso/etendo-25Q2.0.iso) |
| [25.1.2](#2512) | 21/05/2025 | [25Q1.2.iso](https://etendo-appliances.s3.eu-west-1.amazonaws.com/etendo/iso/etendo-25Q1.2.iso) |
| [24.4.3](#2443) | 10/02/2025 | [24Q4.3.iso](https://etendo-appliances.s3.eu-west-1.amazonaws.com/etendo/iso/etendo-24Q4.3.iso) |
| [24.4.0](#2440) | 20/12/2024 | [24Q4.0.iso](https://etendo-appliances.s3.eu-west-1.amazonaws.com/etendo/iso/etendo-24Q4.0.iso) |
| [24.3.0](#2430) | 30/09/2024 | [24Q3.0.iso](https://etendo-appliances.s3.eu-west-1.amazonaws.com/etendo/iso/etendo-24Q3.0.iso) |
| [24.2.0](#2420) | 28/06/2024 | [24Q2.0.iso](https://etendo-appliances.s3.eu-west-1.amazonaws.com/etendo/iso/etendo-24Q2.0.iso) |
| [24.1.4](#2414) | 03/05/2024 | [24Q1.4.iso](https://etendo-appliances.s3.eu-west-1.amazonaws.com/etendo/iso/etendo-24Q1.4.iso) |
| [24.1.0](#2410) | 28/03/2024 | [24Q1.0.iso](https://etendo-appliances.s3.eu-west-1.amazonaws.com/etendo/iso/etendo-24Q1.0.iso) |
| [23.4.0](#2340) | 28/12/2023 | [23Q4.0.iso](https://etendo-appliances.s3.eu-west-1.amazonaws.com/etendo/iso/etendo-23Q4.0.iso) |
| [23.3.0](#2330) | 29/09/2023 | [23Q3.0.iso](https://etendo-appliances.s3.eu-west-1.amazonaws.com/etendo/iso/etendo-23Q3.0.iso) |
| [23.2.0](#2320) | 27/06/2023 | [23Q2.0.iso](https://etendo-appliances.s3.eu-west-1.amazonaws.com/etendo/iso/etendo-23Q2.0.iso) |
| [23.1.0](#2310) | 31/03/2023 | [23Q1.0.iso](https://etendo-appliances.s3.eu-west-1.amazonaws.com/etendo/iso/etendo-23Q1.3.iso) |
| [22.4.5](#2245) | 16/03/2023 | [22Q4.5.iso](https://etendo-appliances.s3.eu-west-1.amazonaws.com/etendo/iso/etendo-22Q4-5.iso) |


## 26.1.1
Nueva ISO con la versión de Etendo 26.1.1. Incluye el script `/utils/apache-config.sh`. Para más información, visite [Configurar Apache](../../../developer-guide/etendo-classic/getting-started/installation/production-server.md#8-configure-apache).

## 25.4.3

- Nueva ISO con la versión de Etendo `25.4.3`
- Añade los bundles **Extensiones de plataforma** y **Extensiones de Copilot**.
- Actualmente, las ISO se basan en la imagen amd64 de Ubuntu Live Server `22.04.5`.
    - python3 → Python `3.10`


## 25.4.0

- Nueva ISO con la versión de Etendo `25.4.0`
- Actualmente, las ISO se basan en la imagen amd64 de Ubuntu Live Server `22.04.5`.

## 25.3.0

- Nueva ISO con la versión de Etendo `25.3.0`
- Actualmente, las ISO se basan en la imagen amd64 de Ubuntu Live Server `22.04.5`.
- Añade **Docker** y **Docker Compose** a la pila de Etendo.

## 25.2.0

- Nueva ISO con la versión de Etendo `25.2.0`
- Actualmente, las ISO se basan en la imagen amd64 de Ubuntu Live Server `22.04.5`.

## 25.1.2

- Nueva ISO con la versión de Etendo `25.1.2`
- Actualmente, las ISO se basan en la imagen amd64 de Ubuntu Live Server `22.04.5`.
- Apache Tomcat `9.0.98`
- Gradle `8.12.1`
- PostgreSQL `16.8.1`
- Java SE `17.0.14`
- Para más información, visite [Documentación de cambios de la API](../../../developer-guide/etendo-classic/developer-changelog/apichanges.md)


## 24.4.3

- Nueva ISO con la versión de Etendo `24.4.3`
- Incidencia [#573](https://github.com/etendosoftware/etendo_core/issues/573) Corrección de problemas de rendimiento de Etendo ISO

## 24.4.0

- Nueva ISO con la versión de Etendo `24.4.0`

## 24.3.0

- Nueva ISO con la versión de Etendo `24.3.0`

## 24.2.0

- Nueva ISO con la versión de Etendo `24.2.0`

## 24.1.4

- Nueva ISO con la versión de Etendo `24.1.4`
- Incidencia [#1](https://github.com/etendosoftware/etendo_backups_script/issues/1) Gradle pierde permisos de ejecución al realizar un etendo-restore

## 24.1.0

- Nueva ISO con la versión de Etendo `24.1.0`
- Actualización de la versión de Tomcat a `9.0.87`

## 23.4.0

- Nueva ISO con la versión de Etendo `23.4.0`

## 23.3.0

- Nueva ISO con la versión de Etendo `23.3.0`

## 23.2.0

- Nueva ISO con la versión de Etendo `23.2.0`

## 23.1.0

- Nueva ISO con la versión de Etendo `23.1.0`

## 22.4.5

- Primera versión ISO con la versión de Etendo `22.4.5`

---
Este trabajo está licenciado bajo :material-creative-commons: :fontawesome-brands-creative-commons-by: :fontawesome-brands-creative-commons-sa: [ CC BY-SA 2.5 ES](https://creativecommons.org/licenses/by-sa/2.5/es/){target="_blank"} por [Futit Services S.L](https://etendo.software){target="_blank"}.