...
Esta sección aplica al módulo “**Modelo AEAT 349 - Declaración recapitulativa de operaciones intracomunitarias**” incluido en el módulo de Localización Española de Etendo.

### **Descripción del módulo**

El nuevo módulo de generación del Modelo 349 como un fichero “.txt” válido de acuerdo con los requisitos establecidos por la Hacienda Española (Orden HAC/360/2002 de 19 de febrero y modificada en última instancia por la Orden HAC/174/2020 de 28 de diciembre) permitirá a las empresas españolas cumplir con sus obligaciones fiscales de presentación de la declaración recapitulativa de las entregas y adquisiciones intracomunitarias de bienes que realicen.
...
Este módulo no tiene en cuenta las transacciones correspondientes al tipo de documento de Etendo “AP/AR credit memo” como facturas de abono, ya que dicho tipo de transacciones no reflejan devoluciones de mercancía. Por ello, el módulo 349 tendrá en cuenta los tipos de documento “AP/AR invoice” (como facturas de compra y venta) y los tipos de documento “AP/AR invoice” negativos (como notas de abono o facturas rectificativas).
...
### **Model 349 Configuration**

This module for generating Model 349 as a valid text file (“\*.txt”) according to the requirements of the Spanish Tax Agency is based on the functionality of the “Tax Report Generator”.

The user can check in the application path: Financial Management / Accounting / Setup / Tax Report, that Model 349 is created as an annual, quarterly, or monthly tax report, all with the same configuration:
...
In the “Report Sections” tab, 4 sections have been created:
...
-   **File** this first section contains an input parameter so the user can enter the name of the 349 file as an input parameter when generating the file.
-   **Identification and Totals** this second section has:
    -   **4 “input” type parameters** that will be shown when generating the 349 so the user can enter them manually:
        -   Contact person: Surname and name of the contact person. This input parameter could be changed to a constant type and, therefore, the value of that constant should be specified, which in this case would be the name of the contact person, for scenarios where the same person submits the report. If so, this parameter would not need to be filled in each time the report is generated.
        -   Contact phone: Phone number of the contact person. This input parameter could be changed to a constant type and, therefore, the value of that constant should be specified, which in this case would be the contact person’s phone number, for scenarios where the same person submits the report. If so, this parameter would not need to be filled in each time the report is generated.
        -   Substitute declaration (yes/no)
        -   Previous declaration identifier: Number of the declaration to be replaced.
    -   **3 “output” type parameters** that the system will retrieve from the database:
        -   Year
        -   Organization name
        -   Organization VAT/Tax ID
-   **Operations** this third section has 2 “output” type parameters that, associated with the corresponding tax rates, will include the operations in the 349:
    -   Intra-community supplies of goods – Tax key E
    -   Intra-community acquisitions of goods – Tax key A
-   **Constants** this fourth section has 11 “constant” type parameters:
    -   Constant in case of annual report (0A)
    -   Constant for the first quarter (1T)
    -   Constant for the second quarter (2T)
    -   Constant for the third quarter (3T)
    -   Constant for the fourth quarter (4T)
    -   349 declaration number (349 is replaced when submitting the file)
    -   Declaration type (349)
    -   Line type 1 (1)
    -   Line type 2 operations (2)
    -   Line type 2 corrections (2)
    -   Submission (T)

### **Tax Configuration**

This new module for generating Model 349 is based on the tax module for Spain and a specific one for 349.

The user can check in the application path: Financial Management / Accounting / Setup / Tax Rate - Tax Parameter tab that the “tax rates/taxes” that must be included in the 349 have been associated with the corresponding 349 tax parameter:

-   The VAT rates for intra-community acquisitions have been associated with the parameter “Intra-community acquisitions of goods” which corresponds to the 349 operation key: “A”
...
-   The VAT rates for intra-community supplies have been associated with the parameter “Intra-community supplies of goods” which corresponds to the 349 operation key: “E”
...
| **186** | Indicador de cambio de periodicidad en la obligación de declarar |
| **187- 390** | Blancos |
...
-   **Declaración 349 mensual** = Una nota de abono de fecha agosto del 2022 que abona una factura de fecha julio del 2022 que ya fue incluida en la declaración del 349 del mes de julio del 2022. La nota de abono de fecha agosto debe incluirse en la declaración del mes de agosto como rectificativa a la declaración del mes de julio para el proveedor/cliente que aplique, indicando además el importe total (base imponible) de compra/venta que se incluyó en la declaración de julio para ese cliente/proveedor.
...
-   **Declaración 349 anual** = Lo mismo aplica en el caso de presentación de la declaración del 349 de forma anual; lo único que varía en este escenario es que, en vez de tener en cuenta meses/trimestres, se tendrá en cuenta el año natural.

Y las que no lo son y deben acumularse para el periodo, por ejemplo:
...
Es por ello que se ha introducido en el sistema una funcionalidad que permite al usuario, a la hora de crear una nota de abono o devolución, indicar si dicha nota de abono/devolución es una “rectificativa del 349” o no; es decir, si rectifica una factura (factura rectificada) de un año/mes/trimestre natural anterior y que, por tanto, ya se incluyó en una declaración del 349 anterior.
...
-   una `AP invoice`/`AR invoice` negativa (devolución de mercancía al proveedor/cliente)
...
-   seleccionar la factura (original) que se está abonando o rectificando (esa selección ya vendrá dada en caso de anulación total de una factura)
-   y además seleccionar el nuevo campo “Rectificativa del 349”, con el fin de indicar además:
    ...
    -   y la base imponible del 349 de servicios o, lo que es lo mismo, el importe total de compra/venta (que para operaciones intracomunitarias coincide con la base imponible total) que para dicho periodo y para dicho proveedor/cliente se informó en el 349 anterior con respecto a la compra/venta de servicios (clave I en compras, clave S en ventas)
...
!!! info
    No todas las notas de abono o devoluciones de mercancía serán rectificativas del 349, solo aquellas que así se configuren por el usuario. 

En caso de que la nota de abono o devolución de mercancía y la factura original pertenezcan al mismo periodo, por ejemplo al primer mes del año 2022 (enero 2022, en caso de declaración del 349 mensual), al primer periodo del año 2022 (enero 2022 a marzo 2022, en caso de declaración del 349 trimestral) o al mismo mes (enero 2022, en el caso de declaración del 349 mensual), tan solo será necesario relacionar ambos documentos en la nueva pestaña “Factura rectificativa” sin tener que seleccionar el parámetro “Rectificativa del 349”.

En este último escenario, el sistema, para un mismo proveedor o cliente y periodo, acumulará las facturas positivas y los abonos con el fin de generar el cómputo global del importe de las transacciones de compra/venta efectuadas con dicho proveedor/cliente (operador intracomunitario) que debe incluirse en la declaración del 349 para un periodo determinado.

### **Presentación del modelo 349 en formato electrónico**

La presentación telemática del modelo 349 en formato electrónico requiere que las empresas tengan un NIF español, así como un certificado electrónico emitido por la “Fábrica Nacional de Moneda y Timbre” (FNMT) u otro certificado válido y reconocido por Hacienda.
...
!!! info
    Existe una “Guía de presentación telemática” en la página web de Hacienda que explica cómo debe realizarse la presentación telemática de las declaraciones y que se puede descargar [*aquí*](https://sede.agenciatributaria.gob.es/static_files/Sede/Procedimiento_ayuda/GI28/instr_mod_349.pdf).
