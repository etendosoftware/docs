...
- Se realiza a partir de la ruta :material-menu: `Gestión Financiera` > `Contabilidad` > `Configuración` > `Rango Impuesto`.
...
- Ruta: :material-menu: `Gestión Financiera` > `Contabilidad` > `Herramientas de análisis` > `Generador de declaraciones de impuestos`.
...
No. El entorno PRE de Veri\*Factu está destinado únicamente a fabricantes de software durante la fase de desarrollo, y no para pruebas de usuarios finales, demos o formaciones. Por este motivo Etendo no permite configurar la conexión a dicho entorno.
