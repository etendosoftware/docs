...
Al crear una factura nueva, el campo **Fecha de Operación** se rellena automáticamente en cualquier vía de creación (ventana de factura, facturación desde albarán, procesos en segundo plano, servicios web/API o creación programática): por defecto toma la **fecha de la factura** y nunca queda vacío para una factura creada con normalidad. Sin embargo, las facturas creadas con una versión del módulo correspondiente anterior a la que incorporó este comportamiento mantienen este campo vacío, lo que impide que el informe dimensional muestre correctamente esos datos. Consulte la sección **Proceso de actualización de Fecha de Operación** en la documentación de [SII](./sii-para-iva.md#proceso-de-actualizacion-de-fecha-de-operacion), [Verifactu](./verifactu.md#proceso-de-actualizacion-de-fecha-de-operacion) o [TicketBAI](./ticketbai-batuz.md#proceso-de-actualizacion-de-fecha-de-operacion) para conocer la versión concreta a partir de la cual aplica en su sistema, y utilice el proceso **Rellenar Fechas de Operación** descrito a continuación para poblarlas.
