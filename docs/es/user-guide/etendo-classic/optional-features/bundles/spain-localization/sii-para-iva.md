...
:octicons-package-16: Paquete Java: `org.openbravo.module.sii`

:octicons-package-16: Paquete Java: `org.openbravo.module.sii.es_es`

:octicons-package-16: Paquete Java: `org.openbravo.module.sii.template`
## **Introducción**

Esta sección explica el contenido del nuevo módulo comercial de Etendo, "**Plantilla Spain AEAT SII**", compatible con Etendo 21Q4 y posteriores, bajo licencia comercial "**Licencia Comercial de Etendo**".
...
Los productos/servicios deben tener correctamente configurado su **"Grupo/Categoría de Impuestos"** (de producto o servicio).

Los terceros (clientes/proveedores) deben tener correctamente configurada su **"Categoría de impuestos de Tercero"**, si aplica, así como su **"CIF/NIF"** o **"Clave NIF País de Residencia"**.

Además, los impuestos deben estar configurados correctamente, tal y como se explica en el siguiente apartado.
...
!!! info
    La utilización del módulo "Spain AEAT SII" requiere de la versión 1.2.204500 o posterior del módulo de impuestos para España, versión que ya incluye toda la configuración de impuestos requerida para el correcto funcionamiento del envío de registros de factura a SII (alta/modificación).
...
| **3 - Pasaporte** | Pasaporte | Para **personas físicas extranjeras** que no tienen NIF ni NIE, pero actúan en operaciones sujetas a IVA (casos poco comunes). | Turista extranjero con factura de venta. |
...
### **Configuración de la Transmisión de Bienes Inmuebles**

Los bienes inmuebles se configuran en la ventana "Producto", asociándolos a la categoría de impuestos "IVA Normal B. Inmuebles", por ejemplo.
...
- **Afectada RECC**, este parámetro permite configurar que, para una Organización no acogida al "Régimen Especial del Criterio de Caja" (RECC), pero sí afectada por recibir facturas de proveedores acogidos, se muestre o no la pestaña de "Cobros/ Pagos Criterio Caja" dentro de la pantalla "Monitor SII".  
...
- será la que, por defecto, se seleccione tanto en facturas de compra como de venta, si además se selecciona el check "**Por defecto**".
...
Si en una factura se selecciona una descripción en el campo "Descripción maestro SII", el campo "Descripción SII" heredará esa descripción para que, para esa operación en concreto, se pueda modificar/completar dicha descripción.
...
- los métodos de pago que configuremos con el tipo "Cheque" serán del tipo 02 = Cheque
- y los métodos de pago que configuremos con el tipo "Otros" serán del tipo 04 = Otros.
...
### **Configuración de alta de factura de compra solo tras su contabilización**

Para organizaciones que desean dar de alta una factura de compra solo tras su contabilización, existe la opción de controlarlo mediante la activación del check "Alta en SII facturas de compra tras su contabilización", ubicado en la ventana Configuración SII, de manera que la fecha de registro contable únicamente se rellenará en el momento en que la factura sea contabilizada, con el día en el que se produzca el asiento contable, independientemente de la fecha imputada en el libro diario, completando así la información necesaria para dar de alta la factura en SII.
...
!!! info
    Si se decide optar por esta funcionalidad, es conveniente que la organización no tenga activo el proceso de contabilización automático, ya que imputará la fecha de registro contable en el momento de realizar la contabilización.
...
En la ventana “Rango de impuesto” se ha creado un nuevo check denominado “Es prorrata”:
...
el detalle de “Más información” que se llamará “Prorrata SII - XML” que estará marcado por
defecto como “Sí”:
...
particularidades que debemos tener en cuenta serán las de indicar en el campo “Índice” el
valor total del impuesto (en los rangos “padres” estándar es un campo en el que se indica 0)
y activar el check “Es prorrata”. En nuestro ejemplo indicaremos en el campo “Índice” el valor
...
“Índice” el valor del porcentaje de impuesto deducible (según nuestro ejemplo indicaremos
7,7) y activar el check “Es prorrata”:
...
en el campo “Índice” el valor del porcentaje de impuesto no deducible (según nuestro

ejemplo indicaremos 2,3) y activar el check “Es prorrata”, tendremos que marcar el check de
“Impuesto no deducible” que se encuentra dentro del detalle “Más información”:
...
Desde Etendo podrá enviar estos registros (alta/modificación) de forma manual o automática conforme a los requerimientos de la AEAT, según se explica a continuación.
### **Envío Manual**
...
- y la "Cadencia Proceso Alta Facturas Venta a SII", establecido para las facturas de venta en la ventana "Configuración SII", para cada Organización
...
Desde la ventana "Monitor SII" podemos gestionar y realizar envíos masivos de facturas emitidas/recibidas a SII, así como de los cobros/pagos de facturas en RECC y los cobros en metálico, siendo esta última información la que se envía de forma anual.
...
Lo mismo aplica para el envío masivo de modificaciones de facturas emitidas/recibidas. En ese caso, dichas facturas deberán encontrarse marcadas como "Modificada por error registral" = Sí.
...
El proceso "Generar facturas (manualmente)” permite crear facturas de venta del tipo "Factura (Cliente)" en estado "Completado".
...
Si el pedido de venta o los pedidos de venta fueran negativos, este proceso generaría facturas de tipo "Factura (Cliente)" que tendremos que reactivar y modificar antes de dar de alta en SII:

- El tipo de documento debe cambiar a un tipo "reverse" como "Revertir factura de venta".
...
- Las **facturas de compra creadas desde pedidos positivos**, se crearán con clave tipo factura "**Factura**", con tipo de documento "Factura (Proveedor)".
- Las **facturas de compra creadas desde pedidos negativos**, se crearán con clave tipo factura "**Factura**", y el usuario debería crearlas con un tipo de documento reverse "Revertir factura de compra", ya que en el caso de las facturas recibidas  
...
La información del registro de alta/modificación de las facturas de venta se obtiene de la ventana "Factura (Cliente)", que como ya se ha explicado se ha modificado para incluir información requerida por SII (fecha de operación, clave de tipo de factura, descripción SII, etc.).

El registro de alta/modificación en SII de las facturas de venta varía en función de las siguientes variables:

- la **localización del "Tercero"** (Cliente), ya sea este extranjero, intracomunitario o nacional, lo que conlleva la sujeción o no al IVA y, dentro de la sujeción, su exención.
...
En este caso, y dado que el IVA se ha devengado en el periodo de agosto (fecha de operación 2 de agosto), el plazo límite de expedición de la factura de venta y su alta en SII sería el 15 de septiembre.
...
- con clave de régimen especial "01" (Régimen General)
- Sujeta no exenta "S1"
...
La organización F&B España emite el 13 de septiembre de 2017 una factura a la Empresa Z situada en Marruecos por la venta de un equipo y el servicio de instalación. La operación se produjo el día 29 de agosto. Este tercero debe identificarse en Etendo con una "Clave NIF País de Residencia", distinta de 1 (NIF) y 2 (NOI).

En este caso, y dado que el IVA se ha devengado en el periodo de agosto (Fecha de Operación 29 de agosto), el plazo límite de expedición de la factura de venta y su alta en SII sería el 13 de septiembre.

Al expedirse la factura con fecha 13 de septiembre, opera fecha límite, es decir, esta factura tendrá que enviarse a SII antes del día 15 de septiembre.
...
![](../../../../../assets/drive/BnMTWAUJv3Itf0gbWYlfNV0RuyNPonFy4GFIpV9Ahh81Y2KK9JiI1p7dVLY79EidOqX8eOuGrGYYwN6Y6cVERAU_iNjULVsQUU_FKxk30SLRq1Nz-WDyW2SSCC_YKC1gDqGXGt87ofjEjs9ka8oiiiQY.png)
...
Este cliente localizado en Francia debe estar identificado en Etendo con una "Clave NIF País de Residencia" de tipo 2 = NOI.

En este caso, y dado que el IVA se ha devengado en el periodo de agosto (fecha de operación 10 de agosto), el plazo límite de expedición de la factura de venta y su alta en SII sería el 15 de septiembre.
...
F&B España se encuentra acogida al Régimen Especial del Criterio de Caja.

Esta organización vende bienes y un servicio a un cliente español el día 6 de julio de 2017, expidiendo la correspondiente factura ese mismo día.

Al registrar esta factura de venta en Etendo, los tipos de impuestos que se seleccionan por defecto serán del tipo:

- Entregas IVA 10% IVA de Caja.
- Prestación de servicios nacional 21% IVA de Caja.
...
- un tipo de Factura "F1"
- con clave de régimen especial "07" (Régimen Especial del Criterio de Caja)
- Sujeta No Exenta "S1"
- con desglose por detalle de IVA.
...
- El comprador que recibe la factura sin IVA declarará este IVA como una Inversión del Sujeto Pasivo. Consulte el escenario de compra Inversión del Sujeto Pasivo.
...
- **Fecha de expedición de la factura** (fecha de factura): 1 de septiembre de 2017
- **Fecha de operación**: 30 de agosto de 2017 (cubriendo el alquiler del mes de agosto).

En este caso, se dispone hasta el 13 de septiembre para dar de alta la factura emitida en SII, contando 8 días naturales desde la fecha de expedición de la factura.
...
- un tipo de Factura "F1"
- con clave régimen especial "12" (Operaciones de arrendamiento de local de negocio no sujetos a retención)
- Sujeta No Exenta "S1"
...
En el caso de locales arrendados sujetos a retención, no es necesario especificar la información del inmueble, por incluirse esta en el modelo de retenciones (Modelo 180) correspondiente.

Por último, recalcar que:

- Si una misma factura incluye más de un arrendamiento con y sin retención, se dará de alta en SII con la clave "13" e información (referencia catastral/situación del inmueble) del/de los local(es) que no estén sujetos a retención.
- Si una factura de arrendamiento está sujeta al Régimen Especial del Criterio de Caja (RECC), prevalecerá la clave 07 de Operación Sujeta a RECC.
...
- como "**Factura**", es decir, con la **"Clave de tipo Factura" = F1**.
  - las facturas de venta positivas, con/sin pedido relacionado, y tercero con CIF/NIF.
- como "**Factura Rectificativa por Diferencias y error fundado"**, es decir, con la "**Clave de tipo Factura" = R1**
  - las facturas de venta negativas, con/sin pedido relacionado, y tercero con CIF/NIF.
- como "**Factura Simplificada**", es decir, con "**Clave tipo Factura" = F2**
  - las facturas positivas o negativas sin/con un ticket/pedido relacionado y tercero sin CIF/NIF.
- como "**Asiento Resumen de Factura Simplificada**", es decir, con "**Clave tipo Factura" = F4**
  - las facturas positivas o negativas con más de un ticket/pedido relacionado, y tercero sin CIF/NIF.
...
#### **Factura emitida rectificativa fuera de plazo**
...
En este caso, y dado que el IVA se ha devengado en el periodo de enero (fecha de operación 2 de enero), el plazo límite de expedición de la factura de venta y su alta en SII sería el 15 de febrero.
...
El día 1 de marzo, se produce una devolución por mal estado del 20% de la mercancía con un valor de 484 € (IVA incluido), por lo que F&B España procede a la anulación de la factura original y a la emisión de una nueva con el importe final correcto.
...
El periodo asignado en SII no se corresponde con la fecha de operación, sino con la Fecha Factura (julio), por lo que **pasará a formar parte de la liquidación del impuesto del mes de julio**.
...
Al igual que la anterior factura que anula el importe de la original, el periodo asignado en SII no se corresponde con la fecha de operación, sino con la Fecha Factura (julio), por lo que **pasará a formar parte de la liquidación del impuesto del mes de julio**.
...
El registro de alta en SII de las facturas de compra varía en función de las siguientes variables:

- la **localización del "Tercero"** (Proveedor), ya sea este extranjero, intracomunitario o nacional
...
- **una operación sujeta a IPSI/IGIC**
- **un arrendamiento de local de negocio** sujeto a IVA
...
- **Fecha de operación** si es diferente a la fecha de la factura
...
Puede indicar el tipo de cambio de la divisa (USD) a EUR en la factura, pestaña "Tipo de cambio", ya que todas las operaciones que se registren en SII deberán estar en euros. De no indicarse tipo de cambio en esta pestaña se tomará el tipo de cambio configurado en la ventana "Rangos de conversión" para la fecha de la factura.
...
- en el grupo de campos "**Importación (DUA)**" introducimos:
  - el **número de DUA**
...
- en el grupo de campos "**AEAT SII**" introducimos la siguiente información:
  ...
  - **Clave tipo factura** como "**Importación (DUA)**" (F5)
  - seleccionamos la "**Descripción SII**" correspondiente
- en este escenario no rellenamos los campos "**Importación (DUA)**" de la cabecera, ya que los datos **número de DUA** y **fecha de admisión del DUA** los vamos a asociar a la línea de impuesto correspondiente al IVA liquidado en aduana.  
...
En estos casos, no se registra factura de transitario, pero sí DUA y, por tanto, el DUA, esto es la deducibilidad del IVA de importación soportado, debe registrarse en Etendo junto a la factura del proveedor extranjero de la siguiente forma:
...
En este caso, al ser la fecha contable de la factura posterior al 16-11-2017, habiéndose realizado esta en octubre, se incluirá en el Modelo 303 de noviembre.
...
- Inversión Sujeto Pasivo UE 21% (para la prestación del servicio y la localización del mismo en España).
...
- si la factura de compra intracomunitaria incluye más de un servicio al mismo/distinto tipo impositivo, en el XML la información se mostrará agrupada por tipo impositivo, dentro del nodo "InversiónSujetoPasivo/DetalleIVA"
#### **Organización/Operación acogida al RECC**

F&B España se encuentra acogida al Régimen Especial del Criterio de Caja, o bien se ve afectada por este régimen al recibir facturas de compra de algún proveedor nacional acogido al criterio de caja.

Al registrar este tipo de factura de compra en Etendo, los tipos de impuestos que se seleccionan por defecto serán del tipo:
...
La compra y la prestación del servicio se efectúa el día 15 de julio; con esa misma fecha se expide la factura, recibiéndose la factura el día 26 de julio. La empresa procede a su registro contable el día 2 de agosto.

- Fecha de Operación: 15 de julio de 2017
- Fecha de expedición de la factura de venta: 15 de julio de 2017
- Fecha de la factura de compra: 15 de julio de 2017
...
- Prestación de servicios en Canarias 9.5%
...
- un tipo de factura "F1"
- con clave de régimen especial "08" (Operaciones sujetas al IPSI / IGIC)
...
- los empleados de la organización que incurran en gastos a reembolsar, tienen que tener la categoría de impuestos de tercero “No declarable en SII” asociada.
...
**Caso 1 = Inversión del Sujeto Pasivo - Proveedor extranjero**
...
- Inversión del Sujeto Pasivo no UE 21%.
...
**Caso 2 = Inversión del Sujeto Pasivo - Proveedor nacional**
...
- Inversión del Sujeto Pasivo nacional 21%.
