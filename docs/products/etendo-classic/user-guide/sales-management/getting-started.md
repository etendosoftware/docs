...
Customers can ask directly for a Sales Order or request a Quotation. If the Business Partner asks for a Quotation Sales staff:
...
- Looks for orders pending for preparation in the [Create Shipments from Orders](/products/etendo-classic/user-guide/sales-management/transactions/?h=create+shipments+orders#create-shipments-from-orders) window or with the help of Orders Awaiting Delivery Report.
- The Warehouse staff can create a Shipment in 2 ways:

With the [Create Shipments from Orders](/products/etendo-classic/user-guide/sales-management/transactions/?h=create+shipments+orders#create-shipments-from-orders) window. It creates a shipment completed for the selected Sales Orders.

With the [Goods Shipment](/products/etendo-classic/user-guide/sales-management/transactions/?h=goods+shipment#goods-shipment) window in which the Warehouse staff creates the shipment in a manual way.
...
- With the [Generate Invoices](/products/etendo-classic/user-guide/sales-management/transactions/?h=generate+invoices#generate-invoices) window, in which it generates invoices in bulk for all pending to be invoiced (based on their invoicing rules) Sales Orders.
- With the [Create Invoices from Orders](/products/etendo-classic/user-guide/sales-management/transactions/?h=invoices+orders#create-invoices-from-orders) window. It shows Orders pending to be invoiced and creates invoices for the selected Sales Orders.
...
!!!info
    For a full description of this functionality visit [How to manage deferred revenue and expenses](http://wiki.openbravo.com/wiki/How_to_manage_deferred_revenue_and_expenses){target="_blank"} article.
...
For more information about payment management documentation visit the [Financial Management//Receivables&Payables](/products/etendo-classic/user-guide/financial-management/receivables-and-payables/transactions/) and in the [How to manage prepaid invoices in receivables](http://wiki.openbravo.com/wiki/How_to_Manage_Prepaid_Invoices_in_Receivables) .
...
- [Return reasons](/products/etendo-classic/user-guide/master-data-management/business-partner-setup/?h=return+reason#return-reasons_1)
- [Condition of the goods](/products/etendo-classic/user-guide/sales-management/setup/?h=condition#condition-of-the-goods)
...
Customers can request a return material for whatever reason.
Sales staff:

- Creates a new document in the [Return from Customer](http://wiki.openbravo.com/wiki/Return_from_Customer#Header){target="_blank"} window and looks for the customer name in the Business Partner field.
...
- With [Return from Customer](http://wiki.openbravo.com/wiki/Return_from_Customer#Header){target="_blank"} window where a new button Create Credit might appear or not based on the original sales order. If the order is already invoiced then it will be present, if not - it won't. Using this button it is possible to invoice the return order following the standard process, that is, depending on the invoice terms.
- With the same [Create Credit](http://wiki.openbravo.com/wiki/Return_from_Customer#Header){target="_blank"} button it is also possible to create an invoice and leave it as credit to be used later.
- Using [Generate Invoices](/products/etendo-classic/user-guide/sales-management/transactions/?h=generate+invoices#generate-invoices) process: if the invoice terms is Customer Schedule After Delivery and both sales orders and RMAs exist the process groups all of them in one standard Sales Invoice (not in a [Return Material Sales Invoices](http://wiki.openbravo.com/wiki/Document_Type){target="_blank"}).
- With the [Sales Invoice](/products/etendo-classic/user-guide/sales-management/transactions/#sales-invoice) window by picking lines and either grouping them from standard sales orders and return orders or by creating individual [Return Material Sales Invoices](http://wiki.openbravo.com/wiki/Document_Type){target="_blank"} only from return orders.
- With the [Sales Invoice](/products/etendo-classic/user-guide/sales-management/transactions/#sales-invoice) window by picking lines and either grouping them from standard sales orders and return orders or by creating individual [Return Material Sales Invoices](http://wiki.openbravo.com/wiki/Document_Type){target="_blank"} only from return orders.
...
- [Warehouse Management](/products/etendo-classic/user-guide/warehouse-management/transactions/) as shipment changes stock quantity and its value.
- [Financial Management](/products/etendo-classic/user-guide/financial-management/receivables-and-payables/transactions/) in terms of managing payments.
- [Material Requirement](/products/etendo-classic/user-guide/material-requirement-planning/transactions/?h=material) (MRP) because pending sales orders are one of the inputs for the prodReversal Sales Invoiceuction process.
