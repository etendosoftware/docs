...
Taxes are applied to Orders and Invoices in two steps:

- **Tax determination**: A default tax is automatically assigned when a product is selected.

- **Tax calculation**: The system calculates estimated amounts during data entry and computes the final values when the document is processed.

!!!info
    Users can manually change the default tax if needed.

#### Default Tax Determination

The default tax is calculated by the `C_GetTax` procedure. The system evaluates several rules to select the most specific and applicable tax:

- **Project tax rate** (for project-related sales)
- **Business partner tax exemption**
- **Product tax category**
- **Business partner tax category compatibility**
- **Tax zones** (region/country specificity)
- **Transaction type** (Sales, Purchases, Both)
- **Cash VAT configuration at document header level**

!!!info
    Taxes defined for more specific criteria (for example, region over country or partner-specific over generic) take precedence.

#### Tax Calculation

- During document editing:

    - An estimated amount is calculated automatically by callouts.

    - Tax information is temporarily stored at header level. 

- During document processing

    When the document is completed:

    - Previous temporary taxes are removed.

    - Taxes are recalculated for accuracy.

    - A tax line is created for each different tax applied.

    - Amounts are calculated based on the taxable base of the related lines. 

For summary taxes, the system calculates amounts using their child taxes, considering cascade behavior if defined.
